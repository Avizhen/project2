...
private_channel = -1001455605183
public_channel = 'pythron' # without @
...
link_text = """Your link is: https://t.me/{bot_username}?start={user_id}"""

first_stage_alert_text = """Someone entered the robot with your link"""

carefully_text = """Choose more carefully"""

select_answer_text = """Select answer"""

active_user_text = """One person was added to your subcategories"""
...
def presenter_works(query = 'null', context = 'null'):
    ...
    bot.send_message(chat_id = presenter, text = translate_text)
...
def link(query, context):
    ...
    code = db.search(info.id == user_id)[0]['lang']
    translate_text = translator.translate(link_text.format(bot_username = bot_username, user_id = user_id), lang_tgt = code)

    query.edit_message_text(text = translate_text)
    query.answer()
...
def test(update, context):
    ...
    user_id = update.effective_user.id
    cha = -1001174724447
    # invite_link = bot.create_chat_invite_link(chat_id = private_channel, member_limit = 1).invite_link

    # url = helpers.create_deep_linked_url(bot.username, str(user_id))
    # output = bot.get_chat_member(chat_id = '@pythron', user_id = user_id)
    # output = bot.get_chat(chat_id = cha)
    code = db.search(info.id == user_id)[0]['lang']
...
def returnfunc(query):
    user_id = query.message.chat.id
    query.message.reply_text(str(query))
    query.answer()
